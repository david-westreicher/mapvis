...
export const HEIGHT_VERTEX_SHADER = `
    ...
    void main() {
        ...
        pos2D = (position.xy + offset);
        uv2D = (position.xy + offset) / 8192.0 + 0.5;

        vec3 heightTex = texture2D(heightTextureCache, physicalTextureCoord(uv2D)).rgb;
        float elevation = ((heightTex.r*256.0 * 256.0 + heightTex.g * 256.0 + heightTex.b)  - 32768.0) / 2000.0 - 0.5;
        vec4 newPosition = vec4(pos2D, elevation, 1.0);
...
export const HEIGHT_FRAGMENT_SHADER = `
    ...
    void main() {
        vec2 textureCoord = physicalTextureCoord(uv2D);
